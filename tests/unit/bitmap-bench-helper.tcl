...
class FakeClient:
    def __init__(self, default="no", strlen=4, bit=1,
                 config_name="bitmap-default-native"):
    ...
    def execute(self, cmd):
        ...
        if name == "CONFIG" and str(cmd[1]).upper() == "GET":
            if str(cmd[2]) != self.config_name:
                return []
            return [self.config_name.encode(), self.default.encode()]
...
client = FakeClient(default="yes", strlen=8, bit=1,
                    config_name="bitmap-default-roaring")
make_bench("native", client).convert_to_native("bench:legacy-config")
assert_no_bitmap_command(client)
assert client.default == "yes", client.commands
assert ["SETBIT", "bench:legacy-config", "0", "1"] in client.commands, client.commands
