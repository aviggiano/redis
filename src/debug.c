...
"BITMAP-CONTAINER-COUNT <key>",
"    Return the number of Roaring containers in a native bitmap key.",
"BITMAP-RAW <key>",
...
NULL
               ...
               c->argc == 3)
    {
        ...
        addReply(c,shared.ok);
    } else if (!strcasecmp(c->argv[1]->ptr,"bitmap-container-count") && c->argc == 3) {
        kvobj *kv = lookupKeyReadOrReply(c, c->argv[2], shared.nokeyerr);
        if (kv == NULL || checkType(c, kv, OBJ_BITMAP)) return;
        addReplyLongLong(c, (long long)bitmapObjectContainerCount(kv));
