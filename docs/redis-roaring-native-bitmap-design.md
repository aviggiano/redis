...
`docs/redis-roaring-migration-contract.md`, and the implementation is
`utils/redis-bitmap-migrate.py`.
...
- **Persistence format and AOF rewrite**: confirm whether the current
  `RDB_TYPE_BITMAP` v2 container-stream payload layout and RESTORE-based AOF
  rewrite are the final upstream shape for this core type. The current
  implementation stores a stable v2 marker plus Redis-owned container records,
  while the loader still accepts legacy raw/range payloads, previous v2
  portable payloads, and the previous byte-length-first layout; open
